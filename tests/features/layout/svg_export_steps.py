...
def step_view_with_elements_in_grid(context, num):
    ...
    for i in range(num):
        ...
        x = col * 150
        y = row * 100
        node = context.view.add(x=x, y=y, w=100, h=50, uuid=f"node-{i}", label=f"Element {i + 1}")
...
def step_view_with_long_name(context, element_name):
    ...
    node = context.view.add(x=10, y=10, w=120, h=55, uuid="node-1", label=element_name)
    assert node is not None
...
def step_view_with_varying_sizes(context, sizes):
    ...
    for i, (w, h) in enumerate(size_list):
        node = context.view.add(x=i * 200, y=0, w=w, h=h, uuid=f"node-{i}", label=f"Node {i + 1}")
        assert node is not None
    ...
    for row in range(2):
        for col in range(3):
            node = context.view.add(
                x=col * 200, y=row * 150, w=100, h=50, uuid=f"node-{row}-{col}", label=f"Element ({row},{col})"
            )
            nodes.append(node)
...
def step_add_connections_with_bendpoints(context):
    ...
    conn = context.view.add_connection(ref=r, source=context.grid_nodes[0], target=context.grid_nodes[5], uuid="conn-1")
...
def step_aggregation_composition_rels(context):
    ...
    try:
        ...
        r_comp = Relationship(
            "Composition", els[0][0].uuid, els[2][0].uuid, name="", uuid="r_comp", parent=context.model
        )
...
def step_svg_has_rectangles_for_elements(context):
    ...
    assert len(node_groups) == num_nodes, f"Expected {num_nodes} node groups, got {len(node_groups)}"
...
def step_svg_has_connection_label(context, label_text):
    ...
    assert label_text in context.svg_string, f"Expected label '{label_text}' in SVG"
...
def step_connection_dashed_5_5(context):
    ...
    assert "5,5" in context.svg_string, "Expected stroke-dasharray 5,5 in SVG"
...
def step_connection_hollow_arrow(context):
    ...
    assert "arrow-hollow" in context.svg_string, "Expected hollow arrow marker in SVG"
...
def step_connection_gray_color(context):
    ...
    assert "#4A4A4A" in context.svg_string or "4a4a4a" in context.svg_string.lower(), "Expected #4A4A4A in SVG"


@then('the connection is labeled "{label}"')
def step_connection_labeled(context, label):
    """Verify connection has the given label."""
    assert label in context.svg_string, f"Expected label '{label}' in SVG"
...
def step_connection_teal_color(context):
    ...
    assert "#4ECDC4" in context.svg_string or "4ecdc4" in context.svg_string.lower(), "Expected #4ECDC4 in SVG"
...
def step_realization_dashed_hollow(context):
    ...
    assert "5,5" in context.svg_string or "2,2" in context.svg_string, "Expected dashed lines for Realization"
...
def step_aggregation_hollow_diamond(context):
    ...
    assert "diamond-hollow" in context.svg_string, "Expected hollow diamond marker for Aggregation"
...
def step_composition_filled_diamond(context):
    ...
    assert "diamond-filled" in context.svg_string, "Expected filled diamond marker for Composition"
...
def step_both_filled_arrow_end(context):
    ...
    assert "arrow-filled" in context.svg_string, "Expected filled arrow markers in SVG"

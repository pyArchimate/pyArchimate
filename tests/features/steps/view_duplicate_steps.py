...
def step_model_with_nodes_and_connections(context):
    ...
    for i in range(5):
        node = context.view.add(
            ref=context.model.add(concept_type="BusinessActor", name=f"Element {i}").uuid, x=i * 150, y=0, w=100, h=50
        )
...
@given('a view named "{view_name}" with 5 nodes and 4 connections')
...
@when('I duplicate the view with name "{new_name}"')
...
@when("modify a node position in the duplicate")
...
@then('a new view is created with name "{expected_name}"')
...
@then("the new view has 5 nodes")
def step_new_view_has_5_nodes(context):
    """Verify the duplicated view has 5 nodes."""
    assert len(context.duplicated_view.nodes) == 5, f"Expected 5 nodes, got {len(context.duplicated_view.nodes)}"


@then("the new view has 4 connections")
def step_new_view_has_4_connections(context):
    """Verify the duplicated view has 4 connections."""
    assert len(context.duplicated_view.conns) == 4, f"Expected 4 connections, got {len(context.duplicated_view.conns)}"


@then("the new view has 5 nodes and 4 connections")
...
@then("both views are registered in the model")
...
@then("the original view is unchanged")
def step_original_view_unchanged(context):
    """Verify the original view is unchanged after duplication."""
    assert context.original_view.name == context.original_name, "Original view name was changed"
    assert len(context.original_view.nodes) == 5, f"Original view nodes changed to {len(context.original_view.nodes)}"
...
def step_original_node_position_unchanged(context):
    ...
    for i, node in enumerate(context.view.nodes):
        ...
        assert node.x == orig_x, f"Original node {i} x position changed from {orig_x} to {node.x}"
        assert node.y == orig_y, f"Original node {i} y position changed from {orig_y} to {node.y}"


@then("the duplicate has the modified position")

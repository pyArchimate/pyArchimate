...
def step_check_all_junctions_preserved(context):
    ...
    for name, original in context.elements.items():
        ...
        if imported and hasattr(original, "get_junction_type"):
            assert original.get_junction_type() == imported.get_junction_type(), f"Junction type mismatch for {name}"
...
def step_check_deep_query(context, path, element_name):
    ...
    assert any(r.uuid == expected.uuid for r in results), f"Expected {element_name} in query results for path {path}"
...
def step_check_still_child(context, element_name, parent_name):
    ...
    parent = context.imported_model.elems_dict.get(context.elements[parent_name.strip("'\"")].uuid)
    child = context.imported_model.elems_dict.get(context.elements[element_name.strip("'\"")].uuid)

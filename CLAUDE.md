...
- Python 3.10+ (per pyproject.toml `requires-python = ">=3.10,<4.0"`) + lxml (ArchiMate XML parsing), existing
  pyArchimate view model, poetry (package manager) (011-view-auto-layout)
...
- 011-view-auto-layout: Added [if applicable, e.g., PostgreSQL, CoreData, files or N/A]
...
- 011-view-auto-layout: Added Python 3.10+ (per pyproject.toml `requires-python = ">=3.10,<4.0"`) + lxml (ArchiMate XML
  parsing), existing pyArchimate view model, poetry (package manager)

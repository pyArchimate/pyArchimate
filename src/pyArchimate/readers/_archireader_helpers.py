...
def _parse_node_type(parent: Any, child: Any, xsi: str) -> Any:
    ...
    if type_n == "DiagramObject":
        ...
        node = parent.add(
            ref=child.get("archimateElement"), uuid=child.get("id"), node_type="Container", label=child.get("name")
        )
...
def _parse_connection(sc: Any, parent: View) -> None:
    ...
    if ref not in parent.model.rels_dict:
        ...
        return
    try:
        conn = parent.add_connection(ref=ref, source=sc.get("source"), target=sc.get("target"), uuid=sc.get("id"))
    ...
    source_node = parent.model.nodes_dict.get(source_id) or parent.model.conns_dict.get(source_id)
    target_node = parent.model.nodes_dict.get(target_id) or parent.model.conns_dict.get(target_id)
    ft = sc.find("feature")
...
def _resolve_rel_endpoints(e: Any, model: Any) -> tuple[Any, Any] | None:
    ...
    if (src_id not in model.elems_dict and src_id not in model.rels_dict) or (
        dst_id not in model.elems_dict and dst_id not in model.rels_dict
    ):
...
def _get_or_create_element(e: Any, model: Any, merge_flg: bool, type_e: str) -> Any:
    ...
    return model.add(concept_type=type_e, name=e.get("name"), uuid=elem_id, profile=e.get("profiles"))
...
def _add_resolved_rel(e: Any, type_e: str, folder: str, model: Any, merge_flg: bool, src: Any, dst: Any) -> None:
    if merge_flg and e.get("id") in model.rels_dict:
        ...
        elem = model.add_relationship(
            rel_type=type_e, name=e.get("name"), uuid=e.get("id"), source=src, target=dst, profile=e.get("profiles")
        )
    elem.folder = folder
...
def get_folders_rel(
    tag: Any, model: Any, xsi: str, merge_flg: bool, folder_path: str = "", unresolved: list[Any] | None = None
) -> None:

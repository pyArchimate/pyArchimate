...
- **VII. System Integrity & Accuracy**: CONDITIONAL PASS, with an open item. The derivation-rule table is the single source of truth for "what counts as implied," unit-tested cell-by-cell so results are reproducible. However — corrected during a post-implementation spec-alignment review — the table's ordering was **not** transcribed directly from a primary copy of the ArchiMate 3.2 §3.5 specification; the Open Group's specification pages required an authenticated session and could not be fetched in this environment. The ordering was instead cross-checked against two independent secondary sources (see research.md Decision 1) and unit-tested for internal consistency, which verifies the table is applied correctly but not that it is the spec's actual table. **Before this feature is treated as fully meeting Principle VII, someone with access to a licensed copy of the ArchiMate 3.2 specification (§3.5) must confirm the `_STRUCTURAL_STRENGTH_ORDER` and `_DYNAMIC_TYPES` combination rules in `src/pyArchimate/derivation.py` against the primary text.** This is tracked as an explicit open item rather than silently assumed correct.

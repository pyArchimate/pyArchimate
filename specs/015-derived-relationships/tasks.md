...
- [X] T003 Write failing unit tests in `tests/unit/test_derivation.py` for a `derive_pair_type(leg1_type, leg2_type)` function, per research.md Decision 1. **Description corrected post-implementation** (original text described 25 structural-type combinations across 5 types including Serving — that was the pre-verification design, since corrected; see T004's note and Known Deviations below). The actual, verified design parametrizes over 16 structural-structural combinations (`Composition`, `Aggregation`, `Assignment`, `Realization` — 4 types, DR2), 4 structural-then-Serving combinations (DR3), 8 structural-then-dynamic combinations (DR5), 4 Triggering-then-structural combinations (DR7), the Triggering+Triggering case (DR8), and explicit `None`-yielding cases for every combination not covered by those rules
...
5. **Stale "weakest-link"/"§3.5" framing left in the docs after the deviation-1 code fix**, found by a second, fresh spec-alignment review requested after deviation 1 landed. The code fix (`src/pyArchimate/derivation.py`) was correct and complete, but the fix's own documentation pass missed updating several places that still described the rule as a single uniform "weakest-link" total order citing "§3.5": `spec.md`'s Assumptions bullet (directly contradicted a second bullet appended two lines below it in the same file), `spec.md` FR-001 and the "Derived Relationship"/"Supported Relationship Type Set" Key Entities, `plan.md`'s Summary, `src/pyArchimate/model.py`'s `check_derivable_duplicates` docstring, and `AI.md` (which contradicted itself within ~200 lines). All corrected to cite ArchiMate 3.2 Specification Appendix B, Section B.2, and to describe the rule set as asymmetric/rule-specific rather than a uniform total order. This T003 entry itself was also corrected (see above) as part of the same pass. Lesson: when a correctness fix touches a widely-cited concept, grep the whole tree for the old framing rather than relying on memory of which files were touched.

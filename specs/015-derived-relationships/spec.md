...
- **FR-001**: System MUST identify, for any model, all two-step relationship chains (element A related to intermediate element B, B related to element C) where both legs use relationship types from the supported subset (Composition, Aggregation, Assignment, Realization, Serving, Triggering, Flow) and where the applicable ArchiMate 3.2 derivation rule (Specification Appendix B, Section B.2) yields a defined implied relationship type between A and C. Not every pairing of two supported-subset types yields a result — see the Supported Relationship Type Set entity below.
...
- **Derived Relationship**: A relationship type, source, and target computed from a qualifying Relationship Chain per the applicable ArchiMate 3.2 derivation rule; exists only transiently as a query/report result, never persisted.
...
- **Supported Relationship Type Set**: The seven relationship types (Composition, Aggregation, Assignment, Realization, Serving, Triggering, Flow) this feature considers eligible for automatic derivation; all other types are out of scope. Eligibility is necessary but not sufficient — not every pairing of two eligible types yields a defined derivation (for example, two elements connected only by a Serving relationship on each leg have no implied relationship between them); which specific pairings do yield a result is an implementation detail, not enumerated here (see research.md).
...
- The derivation rules referenced are the "certain" (valid, not merely potential) derivation rules defined in ArchiMate 3.2 Specification Appendix B, Section B.2, restricted to the seven relationship types the issue explicitly scoped in. These rules are **not** a single uniform "weakest-link" total order across all seven types — they combine structural, dependency, and dynamic relationships via distinct, sometimes asymmetric rules (e.g., a chain of Triggering-then-structural derives Triggering, but Flow-then-structural derives nothing). The exact rule-to-pairing mapping is treated as an implementation detail resolved during planning, not a business requirement to enumerate here (see research.md Decision 1). This rule set was verified on 2026-09-13 against a primary copy of the specification, which superseded and corrected an earlier secondary-source-only version of the table (that version had incorrectly treated `Serving` as a structural relationship rather than a dependency relationship, and had over-generalized dynamic-relationship combination beyond what the specification defines).

...
- **Resolved**: the derivation rule table (which relationship-type pairs derive what) was verified on 2026-09-13 against a primary copy of the ArchiMate 3.2 Specification, Appendix B.2 (DR2, DR3, DR5, DR7, DR8) — see research.md Decision 1. This superseded an earlier secondary-source-only version of the table, which contained two real errors (treating `Serving` as structural rather than a dependency relationship, and over-generalizing dynamic-relationship combination beyond the one rule the specification defines); both were corrected as part of this verification.

...
- **Rules**: `pyArchimate.py` is now a compatibility shim (re-exports only); the core is fully split into `model.py`, `element.py`, `relationship.py`, `view/`, `readers/`, `writers/`, and `helpers/`. Maintain this modular boundary — no new logic should go into `pyArchimate.py`. Every new feature must ship a focused test or well-documented example showing the module it belongs to.
- **Rationale**: Modular core logic is easier to reason about, improves testability, and limits the regression surface when reader/writer behaviors change, preserving trust for automations and scripts.

...
- [ ] T001 Create empty `src/pyArchimate/view/layout/layout_engine.py` with module docstring
- [ ] T002 [P] Create empty `src/pyArchimate/view/layout/routing/obstacle_map.py` with module docstring
- [ ] T003 [P] Create empty `src/pyArchimate/view/layout/routing/segment_separation.py` with module docstring
- [ ] T004 [P] Create empty `tests/unit/view/layout/test_auto_layout.py`
- [ ] T005 [P] Create empty `tests/unit/view/layout/test_auto_route.py`
- [ ] T006 [P] Create empty `tests/unit/view/layout/test_routing_config.py`
- [ ] T007 [P] Create empty `tests/unit/view/layout/test_result_warnings.py`
- [ ] T008 [P] Create empty `tests/integration/test_layout_route_independence.py`
- [ ] T009 [P] Create empty `tests/integration/test_layout_route_chain.py`
...
- [ ] T010 Write failing tests for `RoutingConfig` in `tests/unit/view/layout/test_routing_config.py`: valid defaults, validation rejects `min_segment_gap < 0`, rejects `corner_clearance_pct > 0.50`, rejects `corner_clearance_pct <= 0`, rejects `corner_clearance_min < 0`
...
- [ ] T021 Write failing test: `auto_layout` snaps all node origins to multiples of `LayoutConfig.grid_size` in `tests/unit/view/layout/test_auto_layout.py`
- [ ] T022 [P] Write failing test: `auto_layout` vertical mode — all Business nodes Y < all Application nodes Y < all Technology nodes Y
- [ ] T023 [P] Write failing test: `auto_layout` horizontal mode — all Business nodes X < all Application nodes X < all Technology nodes X
- [ ] T024 [P] Write failing test: no two nodes overlap (bounding boxes do not intersect) after `auto_layout`
- [ ] T025 [P] Write failing test: `auto_layout` does not modify any connection waypoints (SC-010); capture all waypoints before call, compare after
- [ ] T026 Implement layer classification helper in `src/pyArchimate/view/layout/layout_engine.py`: `get_layer_priority(element_type: str) -> int` using ArchiMate category lookup; returns 0–6 per data-model.md layer table; unknown types return 6
- [ ] T027 [P] Implement grid-cell assignment in `src/pyArchimate/view/layout/layout_engine.py`: `assign_grid_cells(nodes, grid_size, layer_direction) -> dict[node_id, tuple[int,int]]`; groups nodes by layer priority; assigns cells in row-major order within each layer band; resolves collisions by advancing to next free cell in row-major order
- [ ] T028 [P] Implement `apply_node_positions(view, cell_assignments, grid_size, margin)` in `src/pyArchimate/view/layout/layout_engine.py`: writes (x, y) to each node from cell assignment; adds `margin` offset; does not touch width/height
...
- [ ] T037 [P] Write failing test: endpoint corner clearance — no endpoint within `max(edge × 0.10, 4px)` of corner
...
- [ ] T049 [P] [US4] Write unit test: `auto_layout` with `grid_size=80` snaps all nodes to multiples of 80 in `tests/unit/view/layout/test_auto_layout.py`
- [ ] T050 [P] [US4] Write unit test: `auto_layout` with `layer_direction="horizontal"` — Business X < Application X < Technology X
- [ ] T051 [P] [US4] Write unit test: `auto_route` with `RoutingConfig(min_segment_gap=20)` — parallel segments separated ≥ 20px
- [ ] T052 [US4] Verify T049–T051 pass; fix config propagation if needed
